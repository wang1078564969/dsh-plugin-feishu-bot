...
/**
 * The route the shipped settings page reads and writes through.
 *
 * Exported rather than registered from here because registration needs the
 * `connection` service, and a Cordis plugin may only reach a service its ENTRY
 * module declared in `inject` — that is `lib/index.js`, not this file, because
 * Cordis imports the entry and never reads the metadata of a module this one
 * imports dynamically. See `lib/index.js` for the wiring.
 */
export const SETTINGS_PATH = '/api/feishu-bot/config'

/**
 * What `lib/index.js` needs in order to mount the settings route.
 *
 * `handler` is filled in by `apply()` below and stays null when the plugin
 * never activated. A module property rather than a service, so nothing else in
 * the composition can come to depend on it.
 */
...
export async function apply(ctx) {
    ...
    /** The last `configSignature()` this instance acted on. */
    let configSignatureSeen = ''

    /**
     * A monotonic counter of adopted configuration changes.
     *
     * The settings page holds a draft copy of every field, so it needs to know
     * whether what it is showing is still current — an operator editing the file
     * by hand, or a second tab, would otherwise be silently overwritten by the
     * next save. It is a signal, not a hash: only "did anything change since
     * revision N". Declared up here, before any user of it, so nothing depends
     * on the order the sections below happen to be written in.
     */
    ...
    /**
     * A cheap "has config.json changed on disk?" signal: mtime in milliseconds
     * plus size.
     *
     * This exists because the operator can now edit the configuration from
     * OUTSIDE this plugin — the settings menu writes the same file — and an
     * in-memory `config` object has no way to learn about that. Polling the file
     * is the only honest answer, and the supervisor already ticks every 3s.
     *
     * Why `statSync` and not the `stat` process the rest of this file uses: this
     * runs on every tick, and a process per tick is a bug this codebase has
     * already paid for once (a `wc` on every log flush). It is also portable,
     * where the `stat -f "%m %N"` used elsewhere is BSD-only.
     *
     * Why milliseconds and not the whole-second `%m`: this plugin writes the
     * file itself (it generates a missing `bridgeToken` on load), and an edit
     * landing in the same second as that write would be invisible forever.
     *
     * `''` means "no such file". The caller only compares, so a config appearing
     * or vanishing counts as a change while an absent file does not churn the
     * reload path on every tick.
     */
    function configSignature() {
      if (!existsSync(CONFIG_PATH)) return ''
      try {
        const info = statSync(CONFIG_PATH)
        return String(Math.round(info.mtimeMs)) + ':' + String(info.size)
      } catch (error) {
        // Unreadable is not "unchanged": return a value no real signature can
        // equal, so the reload path re-runs and loadConfig reports the failure.
        return 'unknown'
      }
    }

    /**
     * One string comparison for two config objects. Values here are strings,
     * booleans, numbers and short arrays, so this is both exact and cheap — and
     * it compares `undefined` on both sides, which a key-by-key walk of one
     * object would get wrong.
     */
    function sameConfigText(a, b) {
      try {
    ...
    /**
     * Adopt an external edit to config.json.
     *
     * Returns nothing; the caller reads the log. The keys that actually differ
     * decide whether the bridge has to be respawned: the child receives appId,
     * appSecret and the endpoint through its ENVIRONMENT at spawn, so a change
     * to any of those without a respawn leaves it on the old identity — inbound
     * events on one app, outbound calls on another.
     *
     * The order matters. `loadConfig` can write the file back (it generates a
     * missing `bridgeToken`), so the signature is captured AFTER it returns:
     * capturing before would see a changed mtime on the next tick and reload
     * forever.
     */
    async function reloadConfigIfChanged() {
      if (configSignature() === configSignatureSeen) return
      const previous = config
      try {
        await loadConfig()
      } catch (error) {
        // loadConfig swallows its own read failures; anything escaping it is a
        // bug, and the supervisor must not die for that.
        log('config reload failed: ' + describeError(error))
        configSignatureSeen = configSignature()
        return
      }
      configSignatureSeen = configSignature()
      ...
      for (let i = 0; i < keys.length; i += 1) {
        if (!sameConfigText(config[keys[i]], previous[keys[i]])) changed.push(keys[i])
      }
      if (changed.length === 0) return
      log('config.json changed on disk — reloaded: ' + changed.join(', '))
      bumpConfigRevision()
      const bridgeFields = ['appId', 'appSecret', 'transport']
      let bridgeAffected = false
      for (const field of bridgeFields) if (changed.indexOf(field) >= 0) bridgeAffected = true
      if (!bridgeAffected) return
      // Kill the child unconditionally; that reconciles BOTH directions. The
      // tick this returns into respawns it when the reloaded transport still
      // wants one, and when it no longer does, leaving the old child alive would
      // keep a long connection open on an identity this plugin no longer claims.
      if (bridge !== null) log('restarting the bridge: ' + changed.join(', ') + ' changed')
      await stopBridge()
      bridgeRestarts = 0
      tokenCache = null
      botInfo = null
     ...
     *
     * `maxBytes` defaults to the 4 MB the Feishu ingress has always allowed.
     * The settings route passes something far smaller: a config patch is a
     * handful of fields, and the default cap would let one request buffer
     * megabytes before being refused.
     */
    function readBody(req, maxBytes) {
      const cap = typeof maxBytes === 'number' && maxBytes > 0 ? maxBytes : 4000000
      return new Promise(function (resolve, reject) {
        ...
        req.on('data', function (chunk) {
          bytes += chunk.length
          if (bytes > cap) {
            reject(new Error('request body exceeded ' + String(cap) + ' bytes'))
            try {
              req.destroy()
            } catch (error) {
              /* already gone */
    ...
    /* ------------------------------------------------------------------ *
     * the settings API — the GUI's window onto config.json
     *
     * WHY A ROUTE AND NOT A TOOL. The settings page ships inside this package
     * as a browser bundle (`lib/client.js`, declared by `dsh.client` in
     * package.json), and a SHIPPED client half has no `host.call`: that is a
     * builtin of the dynamic Cordis runner only. The zero-build way for a
     * shipped browser half to reach its own host half is an exact Fetch route
     * on the connection service, which is also the only way to get the
     * browser-cookie authentication: a raw `webServer` route would answer any
     * local process, and this endpoint reports whether the app secret is set.
     *
     * WHY NOT `feishu_bot action: configure`. That action applies the same
     * patch, but it is a MODEL tool — reaching it from here would mean calling
     * the tool registry, and the page would then have to reproduce this
     * plugin's own merge rules to know what it had changed. One code path for
     * "change the config" is the point.
     * ------------------------------------------------------------------ */

    /** Body cap for one settings request. The payload is a handful of fields. */
    const SETTINGS_MAX_BODY = 65536

    /**
     * Settings keys the page may write, mirroring the field list it renders.
     *
     * `bridgeToken` is deliberately absent: this plugin generates it, nothing
     * outside should be able to set or clear it. So are the two arrays, which
     * the page edits as comma-separated text only when it is the one that
     * changed them — an empty string there means "no restriction", which the
     * validator below has to be able to express.
     */
    const SETTINGS_KEYS = [
      'appId', 'appSecret', 'verificationToken', 'encryptKey', 'transport',
      'workspacePath', 'agentPreset', 'permissionPreset', 'groupRequireMention',
      'replyStyle', 'acknowledge', 'cardHeader', 'replyMetrics', 'maxReplyChars',
      'timeoutMs', 'useReply', 'allowedChatIds', 'blockedUserIds',
    ]

    /** Never sent to a browser, whatever the caller asks for. */
    const SETTINGS_SECRETS = ['appSecret', 'encryptKey', 'verificationToken', 'bridgeToken']

    /**
     * One config value as JSON-safe data.
     *
     * The RPC clone in the dynamic runner rejects `undefined` outright, and this
     * route has the same obligation for a different reason: an absent key is a
     * normal state (this plugin writes back only values that differ from its
     * defaults), and `null` is the honest wire value for it. A value that is
     * not JSON at all must not reach a browser either.
     */
    function jsonSafe(value) {
      ...
      if (Array.isArray(value)) {
        const out = []
        for (let i = 0; i < value.length; i += 1) out.push(jsonSafe(value[i]))
        return out
      }
      return String(value)
    }
    ...
    /**
     * The configuration as the page should see it: every editable key present,
     * secrets reduced to "is it set", and nothing `undefined`.
     */
    function settingsView() {
      const fields = {}
      for (let i = 0; i < SETTINGS_KEYS.length; i += 1) {
        const key = SETTINGS_KEYS[i]
        const value = Object.prototype.hasOwnProperty.call(config, key) ? config[key] : DEFAULTS[key]
        if (SETTINGS_SECRETS.indexOf(key) >= 0) {
          fields[key] = null
          continue
        }
      ...
      const secretsSet = []
      for (let i = 0; i < SETTINGS_SECRETS.length; i += 1) {
        const key = SETTINGS_SECRETS[i]
        if (typeof config[key] === 'string' && config[key] !== '') secretsSet.push(key)
      }
      return { fields: fields, secretsSet: secretsSet }
    }

    /**
     * Validate one incoming patch and return the writes to apply.
     *
     * Every rejection is reported rather than silently dropped, because the
     * page shows these lines to the operator. Two rules are load-bearing:
     * an empty secret means "leave it alone" (so a form that never received the
     * secret cannot erase it), and `transport` is a closed set — a typo there
     * would otherwise leave a bridge that neither dials out nor listens.
     */
    function validateSettingsPatch(patch) {
      ...
      for (let i = 0; i < keys.length; i += 1) {
        const key = keys[i]
        if (SETTINGS_KEYS.indexOf(key) < 0) {
          notices.push('ignored unknown key: ' + key)
          continue
        }
        let value = patch[key]
        if (SETTINGS_SECRETS.indexOf(key) >= 0) {
          if (typeof value !== 'string' || value === '') {
            notices.push(key + ' left empty = unchanged')
            continue
          }
        }
        if (key === 'appId' && (typeof value !== 'string' || value.trim() === '')) {
          notices.push('appId cannot be empty')
          continue
        }
        if (key === 'transport') {
          if (value !== 'ws' && value !== 'webhook' && value !== 'both') {
            notices.push('transport refuses ' + JSON.stringify(value) + ' — ws, webhook or both')
            continue
          }
        }
        if ((key === 'workspacePath') && typeof value === 'string' && value !== '' && value.charAt(0) !== '/') {
          notices.push('workspacePath needs an absolute path: ' + value)
          continue
        }
        if (typeof value === 'string') value = value.trim()
        write[key] = value
      }
      return { write: write, notices: notices }
    }

    /**
     * The route's request handler: GET reads, POST writes.
     *
     * FETCH-SHAPED, not node:http. The connection service hands its routes a
     * WHATWG `Request` and expects a `Response`; the Feishu ingress above uses
     * node:http because that route belongs to `webServer`, a different registry.
     * Staying in the shape this registry speaks avoids an adapter that would
     * have to fake `setHeader`/`end` just to be unwrapped again.
     *
     * Every reply carries the configuration AS PERSISTED, so the page shows what
     * is on disk: a save that silently did not happen cannot look like one that
     * did.
     */
    async function settingsRequest(request) {
      const json = function (status, payload) {
        let text = '{}'
        try {
          text = JSON.stringify(payload)
        } catch (error) {
          text = '{"ok":false,"error":"the response was not serializable"}'
        }
      ...
      try {
        ...
        if (method === 'GET') {
          ...
          return json(200, { ok: true, fields: view.fields, secretsSet: view.secretsSet, revision: configRevision() })
        }
        if (method !== 'POST') return json(405, { ok: false, error: 'GET or POST only' })

        const raw = await request.text()
        if (raw.length > SETTINGS_MAX_BODY) return json(413, { ok: false, error: 'body too large' })

        let payload
        try {
          payload = JSON.parse(raw)
        } catch (error) {
          return json(400, { ok: false, error: 'invalid JSON: ' + describeError(error) })
        }
        const patch = payload !== null && typeof payload === 'object' && payload.updates !== null && typeof payload.updates === 'object' && !Array.isArray(payload.updates)
          ? payload.updates
          : null
        if (patch === null) return json(400, { ok: false, error: 'updates must be an object' })

        const checked = validateSettingsPatch(patch)
        const writeKeys = Object.keys(checked.write)
        if (writeKeys.length === 0) return json(400, { ok: false, error: 'nothing to write', notices: checked.notices, fields: null, secretsSet: [] })

        /*
         * Adopt the patch into the LIVE config object, then persist.
         *
         * Adopting first is what makes the page immediate: every feature reads
         * `config.x` at use time, so a reply-style change applies to the next
         * reply without waiting for the 3-second watcher to notice the file. The
         * watcher still runs and sees its own write as a no-op, because the
         * signature is captured after loadConfig returns.
         *
         * The three connection fields are the exception no page can paper over:
         * the bridge child receives them through its ENVIRONMENT at spawn, so
         * they only take effect on a respawn. The supervisor does that on its own
         * — `bridgeWanted()` re-reads the transport — and the reply says so
         * rather than implying the change is already live.
         */
        let bridgeAffected = false
        for (let i = 0; i < writeKeys.length; i += 1) {
          const key = writeKeys[i]
          config[key] = checked.write[key]
          if (key === 'appId' || key === 'appSecret' || key === 'transport') bridgeAffected = true
        }
        ...
        return json(200, {
          ...
          bridgeAffected: bridgeAffected,
          revision: configRevision(),
        })
      } catch (error) {
        log('settings request failed: ' + describeError(error))
        return json(500, { ok: false, error: describeError(error) })
      }
    }
    ...
    await loadConfig()
    await loadState()
    // Seed the change signal with the file as it is NOW, so the first tick does
    // not treat this very load as an external edit and reload a second time.
    configSignatureSeen = configSignature()
    ...
    ctx.interval(function () {
      ...
      // Adopt an external edit to config.json BEFORE the bridge checks below, so
      // this very tick acts on the new values: a transport that changed to
      // `webhook` returns here instead of respawning a long connection, and a
      // changed appId or appSecret is picked up by the respawn rather than
      // leaving the child running on the old identity.
      reloadConfigIfChanged().catch(function (error) {
        log('config reload check failed: ' + describeError(error))
      })
    ...
    /*
     * Hand the settings handler out to the entry module, which owns the
     * `connection` registration. Done at the END of apply so a handler is only
     * ever published by an instance that actually finished activating.
     */
    settingsApi.handler = settingsRequest
